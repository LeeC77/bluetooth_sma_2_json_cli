...
- Build from for Python source cade files (if your OS is not supported).
  For Linux ```wget https:// TODO``` 

  For Windows ```curl "https:// TODO```
...
- Build the executable ```pyinstaller .\main.py --onefile -n.\sma2json.exe```
...
**Don't forget to make 'sma2json.exe' executable and set the ownership, if your OS requires it. ```chmod +x sma2json.exe``` and ```chown ``` TODO.**
